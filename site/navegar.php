...
<head>
  ...
  <link rel="stylesheet" href="navegar.css">
  ...
  <main> <!-- Inicio do Corpo do site -->
    ...
    <section class="section-1"> <!-- Inicio do conteudo Primario -->
      ...
      <div class="container-fluid SC">
        <div id="filtro">
          <b>Filtrar PCCT's por: </b>
          <form action="navegar.php" method="post">
            Curso:
            <input type="radio" name="secso" value="m">Informática
            <input type="radio" name="secso" value="f">Administração
            <input type="radio" name="secso" value="f">Agropecuaria
            <br> <br>
            <input type="text" name="nome_arquivo" placeholder="Pesquisar nome doPCCT..."><br>
        </div>
        ...
        $select = "SELECT * FROM pcct_pdf";
        if (!empty($_POST)) {
          $select .= " WHERE (1=1) ";
          if (isset($_POST["secso"])) {
            $secso = $_POST["secso"];
            $select .= "AND secso = '$secso'";
          }
          if ($_POST["nome_arquivo"] != "") {
            $nome_arquivo = $_POST["nome_arquivo"];
            $select .= " AND nome_arquivo like '%$nome_arquivo' ";
          }
        }
        $select .= " ORDER BY nome_arquivo";
        $resultado = mysqli_query($conexao, $select);
        while ($linha = mysqli_fetch_assoc($resultado)) {
          echo "<b>ID do PCCT:</b>" . $linha["id_pcct"] . "<br />";
          echo "<b>Nome do PCCT:</b>" . $linha["nome_arquivo"] . "<br />";
          echo "<b>PCCT em PDF:</b>" . $linha["conteudo_pdf"] . "<br />";
          echo "<b>Data de envio :</b>" . $linha["dataUpload"] . "<br />";
          // Atenção - Filtro
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Pesquisar de pccts</title>
</head>
